<?php
$browser_language = (string) (isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) ===
    true && $_SERVER['HTTP_ACCEPT_LANGUAGE'] !== '')
    ? strtok(strip_tags($_SERVER['HTTP_ACCEPT_LANGUAGE']), ',')
    : '';
$browser_language =
    isset($_GET['language']) === true && $_GET['language'] !== ''
        ? $_GET['language']
        : $browser_language;
$language = (string) '';
...
<html lang="<?php echo $language; ?>">
  ...
  <body>
    <?php if ($language === 'de'): ?>
      ...
      <p class="text">
        <span lang="en">Server-Software</span>: <samp><?php echo strip_tags(
            htmlentities($_SERVER['SERVER_SOFTWARE'], ENT_QUOTES, 'UTF-8')
        ); ?></samp><br>
        <span lang="en">Server-Name</span>: <samp><?php echo strip_tags(
            htmlentities($_SERVER['SERVER_NAME'], ENT_QUOTES, 'UTF-8')
        ); ?></samp><br>
        <span lang="en">Document-Root</span>: <samp><?php echo strip_tags(
            htmlentities($_SERVER['DOCUMENT_ROOT'], ENT_QUOTES, 'UTF-8')
        ); ?></samp><br>
        Protokoll: <samp><?php echo isset($_SERVER['HTTPS']) === true &&
        $_SERVER['HTTPS'] !== 'off'
            ? '<abbr lang="en" title="Hypertext Transfer Protocol Secure">https</abbr>'
            : '<abbr lang="en" title="Hypertext Transfer Protocol">http</abbr>'; ?></samp><br>
    ...
    <?php elseif ($language == 'en'): ?>
      ...
      <p class="text">
        Server software: <samp><?php echo strip_tags(
            htmlentities($_SERVER['SERVER_SOFTWARE'], ENT_QUOTES, 'UTF-8')
        ); ?></samp>  <br>
        Server name: <samp><?php echo strip_tags(
            htmlentities($_SERVER['SERVER_NAME'], ENT_QUOTES, 'UTF-8')
        ); ?></samp>  <br>
        Document root: <samp><?php echo strip_tags(
            htmlentities($_SERVER['DOCUMENT_ROOT'], ENT_QUOTES, 'UTF-8')
        ); ?></samp>  <br>
        Protocol: <samp><?php echo isset($_SERVER['HTTPS']) === true &&
        $_SERVER['HTTPS'] !== 'off'
            ? '<abbr title="Hypertext Transfer Protocol Secure">https</abbr>'
            : '<abbr title="Hypertext Transfer Protocol">http</abbr>'; ?></samp><br>
